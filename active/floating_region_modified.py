...
class FloatingRegionScore(nn.Module):
    ...
    def forward(self, logit):
        ...
        """
        p = torch.softmax(logit, dim=1)  # [B,C, h, w]

        # pixel_entropy = torch.sum(-p * torch.log(p + 1e-6), dim=0).unsqueeze(dim=0).unsqueeze(dim=0) / math.log(
        #     19)  # [1, 1, h, w]
        pixel_entropy = torch.sum(-p * torch.log(p + 1e-6), dim=1, keepdim=True) # [B, 1, h, w]
        region_sum_entropy = self.entropy_conv(pixel_entropy)  # [B, 1, h, w]

        predict = torch.argmax(p, dim=1)  # [B,h, w]

        one_hot = F.one_hot(predict,num_classes=self.in_channels).float() # [B, h, w, C]
        one_hot = one_hot.permute((0,3, 1, 2)) # [B,C, h, w]

        summary = self.purity_conv(one_hot)  # [B, C, h, w]
        count = torch.sum(summary, dim=1, keepdim=True)  # [B, 1, h, w]
        dist = summary / count  # [B,C, h, w]
        # region_impurity = torch.sum(-dist * torch.log(dist + 1e-6), dim=1, keepdim=True) / math.log(19)  # [1, 1, h, w]
        region_impurity = torch.sum(-dist * torch.log(dist + 1e-6), dim=1, keepdim=True) # [B,1, h, w]
        prediction_uncertainty = region_sum_entropy / count  # [B, 1, h, w]

        score = region_impurity * prediction_uncertainty
        return score.squeeze(dim=1), region_impurity.squeeze(dim=1), prediction_uncertainty.squeeze(dim=1)
